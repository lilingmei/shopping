...
export default {
  ...
  methods: {
    ...
    // 点击添加到购物车
    addFirst (event) {
      // 添加商品的同时还需要指明表示【哪个小球】的【目标target】
    ...
    // cartcontrol组件的@add后绑定的方法
    // 在该方法的参数中可以获取到cartcontrol组件中$emit的目标target
    addFood (target) {
      this.$emit('add', target)
    },
    // 显示
    needShow (type, text) {
      // 如果勾选了【只看有内容的评价】并且【文本不存在】
